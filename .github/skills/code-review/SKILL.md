---
name: code-review
description: "Review pull requests and diffs in ixpert-for-ipad — the preserved iPad edition of the iXpert converter in manual-reference-counted Objective-C — against the rios0rios0/guide standards, with extra weight on conversion correctness, the iPad-specific presentation, and the button image naming. Use when reviewing a PR, a branch, or staged changes here."
---

# Code review — `ixpert-for-ipad`

`iXpert for iPad` is the tablet adaptation of the iXpert converter: the same encode/decode modes, presented with an action sheet and a popover for the how-to page. It is a **preserved historical archive** built with Xcode 4 against iOS SDK 5, using manual reference counting.
...
## What matters most in `ixpert-for-ipad`

These are the checks that catch real defects in this repository. Work through
them before the generic ones.

- **Conversion correctness is the product.** Every mode needs its boundaries checked: empty input, odd-length hex, invalid Base64 padding, non-ASCII characters, and the one-way modes staying one-way. A silent wrong answer is the worst failure here.
- **Popovers and action sheets are iPad-specific and stateful.** A popover presented while another is visible, or one that is not dismissed when its anchor disappears, raises at runtime. Check the presentation and dismissal paths together.
- **Manual reference counting, everywhere.** Balance every `retain` with a `release`; no ARC, no Swift. **Critical** when unbalanced, and expect `-fno-objc-arc` on the `.m` files with a modern Xcode.
- **Button images follow `Button <Mode> Norm.png` / `Button <Mode> Sele.png`.** A new mode needs both states, at the right size, under `Imgs/Buttons/` — a missing selected image shows an invisible button in the pressed state.
- **Only Apple system frameworks** (UIKit, Foundation, CommonCrypto). There is no CocoaPods, Carthage, or SwiftPM here and none should be added.
- **UI layout is in XIB files** — an outlet renamed in the header without the XIB update crashes at load while compiling cleanly.
- **Keep it in step with [`ixpert`](https://github.com/rios0rios0/ixpert)** where the conversion logic is shared in spirit: a fix to a converter here is usually a fix there too, and the review should say so.
- **Treat this as an archive** — no ARC migration, no Storyboards, no auto-layout, no reformatting.

### Commands a reviewer should be able to quote

```bash
# open "Project/iXpert for iPad.xcodeproj" in Xcode, target an iPad Simulator, Cmd+R
# with a modern Xcode: add -fno-objc-arc under Build Settings → Other C Flags
```

## Tests

There are no XCTest targets. Verification is manual, in an iPad simulator: tap each mode and confirm it highlights, encode and decode a value, open the information action sheet, and open and dismiss the how-to popover.
